...
/* When the YouTube API code loads, it calls this function, so it must be global and it must be named exactly onYouTubeIframeAPIReady. */
window.onYouTubeIframeAPIReady = function () {
  var videoModules = document.querySelectorAll('.video-main');
  if(!videoModules) return;
...
class instagramSlider extends HTMLElement{
  constructor(){
    ...
    this.instagramSwiper = this.querySelector('.swiper');
    this.navNext = this.querySelector('.insta-nav-next');
    this.navPrev = this.querySelector('.insta-nav-prev');

    this.init();
  }

  init(){
    this.instagramSlider = new Swiper(this.instagramSwiper, {
      slidesPerView: this.slidesPerView,
      spaceBetween: 0,
      navigation: {
        nextEl: this.navNext,
...
class CounterUp extends HTMLElement{
  constructor(){
    ...
    this.counterList = this.querySelector('.counter-up-list');
    this.activated = false;

    const observer = new IntersectionObserver((entries, observer) => {
      if (!entries[0].isIntersecting) return;
      observer.unobserve(this.counterList);
      this.updateCounter();
    });
    
    observer.observe(this.counterList);
  }
...
class filterDrawer extends HTMLElement {
  constructor() {
    ...
    this.filterWrapper = this.querySelector(".filter-sidebar");
    this.filterButtons = this.querySelectorAll(".filter-opener");

    this.filterButtons.forEach(btn => {
      btn.addEventListener('click', this.filterToggle.bind(this));
    });
  }

  filterToggle(){
    this.filterWrapper.classList.toggle("open-sidebar");
  }
}
